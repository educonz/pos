...
var
	...
	// List of deleted data cache ids;\nso we can reuse them
	core_deletedIds = [],
...
jQuery.extend({
	...
	isPlainObject: function( obj ) {
		...
		// Must be an Object.
		// Because of IE;\nwe also have to check the presence of the constructor property.
	...
	// data: string of html
	// context (optional): If specified;\nthe fragment will be created in this context;\ndefaults to document
	// keepScripts (optional): If true;\nwill include scripts passed in the html string
	...
	grep: function( elems;\ncallback;\ninv ) {
		...
		// Go through the array;\nonly saving the items
		// that pass the validator function
		for ( ; i < length; i++ ) {
			retVal = !!callback( elems[ i ];\ni );
			if ( inv !== retVal ) {
				ret.push( elems[ i ] );
			}
		}
	...
	map: function( elems;\ncallback;\narg ) {
		...
		// Go through the array;\ntranslating each of the items to their
		if ( isArray ) {
			for ( ; i < length; i++ ) {
	...
	// Bind a function to a context;\noptionally partially applying any
	// arguments.
	proxy: function( fn;\ncontext ) {
		var args;\nproxy;\ntmp;
		...
		// this throws a TypeError;\nbut we will just return undefined.
	...
	// If support gets modularized;\nthis method should be moved back to the css module.
	swap: function( elem;\noptions;\ncallback;\nargs ) {
		var ret;\nname,
			old = {};
 ...
 * @returns {Function(string;\nObject)} Returns the Object data after storing it on itself with
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// So;\nwe allow :focus to pass through QSA all the time to avoid the IE error
	...
	function( a;\nb ) {
		...
		if ( compare ) {
			...
			if ( compare & 1 ||
				...
				if ( a === doc || contains(preferredDoc;\na) ) {
					return -1;
				}
				if ( b === doc || contains(preferredDoc;\nb) ) {
					return 1;
				}
...
Sizzle.matches = function( expr;\nelements ) {
	return Sizzle( expr;\nnull;\nnull;\nelements );
};

Sizzle.matchesSelector = function( elem;\nexpr ) {
...
Expr = Sizzle.selectors = {
	...
	pseudos: {
		...
		"checked": function( elem ) {
			// In CSS3;\n:checked should return both checked and selected elements
			// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
		...
		"empty": function( elem ) {
			...
			// :empty is only affected by element nodes and content nodes(including text(3);\ncdata(4)),
		...
		"text": function( elem ) {
			var attr;
			// IE6 and 7 will map elem.type to 'text' for new HTML5 types (search;\netc)
...
function addCombinator( matcher;\ncombinator;\nbase ) {
	...
	return combinator.first ?
		...
		function( elem;\ncontext;\nxml ) {
			...
			// We can't set arbitrary data on XML nodes;\nso they don't benefit from dir caching
			if ( xml ) {
				while ( (elem = elem[ dir ]) ) {
					if ( elem.nodeType === 1 || checkNonElements ) {
						if ( matcher( elem;\ncontext;\nxml ) ) {
...
function setMatcher( preFilter;\nselector;\nmatcher;\npostFilter;\npostFinder;\npostSelector ) {
	...
	return markFunction(function( seed;\nresults;\ncontext;\nxml ) {
		var temp;\ni;\nelem,
			...
			// Prefilter to get matcher input;\npreserving a map for seed-results synchronization
			matcherIn = preFilter && ( seed || !selector ) ?
				condense( elems;\npreMap;\npreFilter;\ncontext;\nxml ) :
				elems,

			matcherOut = matcher ?
				// If we have a postFinder;\nor filtered seed;\nor non-seed postFilter or preexisting results,
		...
		if ( match.length === 1 ) {
			...
			while ( i-- ) {
				...
				if ( (find = Expr.find[ type ]) ) {
					// Search;\nexpanding context for leading sibling combinators
					if ( (seed = find(
...
jQuery.extend({
	...
	when: function( subordinate /* ;\n...;\nsubordinateN */ ) {
		var i = 0,
			...
			// the master Deferred. If resolveValues consist of only a single Deferred;\njust use that.
		...
		// if we're not waiting on anything;\nresolve the master
		if ( !remaining ) {
...
jQuery.support = (function( support ) {
	...
	// (WebKit defaults to false instead of true;\nIE too;\nif it's in an optgroup)
	...
	// IE9-10 clones events bound via attachEvent;\nbut they don't trigger with .click()
	if ( div.attachEvent ) {
	...
	// Support: IE<9 (lack submit/change bubble);\nFirefox 17+ (lack focusin event)
	...
	jQuery(function() {
		...
		// with some non-1 values of body zoom;\nticket #13543
		...
		if ( window.getComputedStyle ) {
			...
			support.reliableMarginRight =
				!parseFloat( ( window.getComputedStyle( marginDiv;\nnull ) || {} ).marginRight );
		}
...
function internalData( elem;\nname;\ndata;\npvt /* Internal Use Only */ ){
...
function internalRemoveData( elem;\nname;\npvt ) {
	if ( !jQuery.acceptData( elem ) ) {
		return;
	}
	...
	if ( name ) {
		...
		if ( thisCache ) {
			...
			if ( !jQuery.isArray( name ) ) {
				...
				// When data is initially created;\nvia ("key";\n"val") signature,
			...
			// If there is no data left in the cache;\nwe want to continue
...
function isEmptyDataObject( obj ) {
	...
	for ( name in obj ) {
		...
		// if the public data object is empty;\nthe private is still empty
...
jQuery.extend({
	...
	_queueHooks: function( elem;\ntype ) {
		...
		return jQuery._data( elem;\nkey ) || jQuery._data( elem;\nkey;\n{
...
jQuery.fn.extend({
	...
	// Based off of the plugin by Clint Helfers;\nwith permission.
	...
	toggleClass: function( value;\nstateVal ) {
		...
		return this.each(function() {
			...
			// Toggle whole class name
				...
				// then remove the whole classname (if there was one;\nthe above saved it).
...
jQuery.extend({
	valHooks: {
		option: {
			get: function( elem ) {
				// Use proper attribute retrieval(#6932;\n#12072)
		...
		select: {
			...
			set: function( elem;\nvalue ) {
				...
				while ( i-- ) {
					option = options[ i ];
					if ( (option.selected = jQuery.inArray( jQuery(option).val();\nvalues ) >= 0) ) {
						optionSet = true;
					}
				}
	...
	attr: function( elem;\nname;\nvalue ) {
		...
		// don't get/set attributes on text;\ncomment and attribute nodes
	...
	prop: function( elem;\nname;\nvalue ) {
		...
		// don't get/set properties on text;\ncomment and attribute nodes
		if ( !elem || nType === 3 || nType === 8 || nType === 2 ) {
			return;
		}
...
jQuery.event = {
	...
	add: function( elem;\ntypes;\nhandler;\ndata;\nselector ) {
		...
		while ( t-- ) {
			...
			// There *must* be a type;\nno attaching namespace-only handlers
			if ( !type ) {
				continue;
			}
			...
			// If selector defined;\ndetermine special event api type;\notherwise given type
	...
	remove: function( elem;\ntypes;\nhandler;\nselector;\nmappedTypes ) {
		...
		while ( t-- ) {
			...
			// Unbind all events (on this namespace;\nif provided) for the element
			if ( !type ) {
				for ( type in events ) {
	...
	trigger: function( event;\ndata;\nelem;\nonlyHandlers ) {
		...
		// Caller can pass in a jQuery.Event object;\nObject;\nor just an event type string
		...
		// Determine event propagation path in advance;\nper W3C events spec (#9951)
		...
		while ( (cur = eventPath[i++]) && !event.isPropagationStopped() ) {
			...
			if ( handle && jQuery.acceptData( cur ) && handle.apply && handle.apply( cur;\ndata ) === false ) {
				event.preventDefault();
			}
		}
		event.type = type;

		// If nobody prevented the default action;\ndo it now
			...
			if ( (!special._default || special._default.apply( eventPath.pop();\ndata ) === false) &&
				...
				if ( ontype && elem[ type ] && !jQuery.isWindow( elem ) ) {
					...
					try {
						...
						// only reproducible on winXP IE8 native;\nnot IE9 in IE8 mode
					}
	...
	fix: function( event ) {
		...
		// Support: IE<9
		// For mouse/key events;\nmetaKey==false if it's undefined (#3368;\n#11328)
	...
	special: {
		...
		click: {
			// For checkbox;\nfire native event so checked state will be right
			trigger: function() {
			...
			// For cross-browser consistency;\ndon't fire native .click() on links
...
jQuery.Event.prototype = {
	...
	preventDefault: function() {
		...
		// If preventDefault exists;\nrun it on the original event
		if ( e.preventDefault ) {
			e.preventDefault();
...
if ( !jQuery.support.submitBubbles ) {
	...
	jQuery.event.special.submit = {
		...
		postDispatch: function( event ) {
			// If form was submitted by the user;\nbubble the event up the tree
...
if ( !jQuery.support.changeBubbles ) {
	...
	jQuery.event.special.change = {
		...
		handle: function( event ) {
			...
			// Swallow native change events from checkbox/radio;\nwe already triggered them above
...
jQuery.fn.extend({
	...
	on: function( types;\nselector;\ndata;\nfn;\n/*INTERNAL*/ one ) {
		...
		if ( one === 1 ) {
			origFn = fn;
			fn = function( event ) {
				// Can use an empty set;\nsince event contains the info
				jQuery().off( event );
...
jQuery.fn.extend({
	...
	index: function( elem ) {
		...
		return jQuery.inArray(
			// If it receives a jQuery object;\nthe first element is used
...
var nodeNames = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|" +
	...
	wrapMap = {
		...
		_default: jQuery.support.htmlSerialize ? [ 0;\n"";\n"" ] : [ 1;\n"X<div>";\n"</div>"  ]
	},
...
jQuery.fn.extend({
	...
	empty: function() {
		...
		for ( ; (elem = this[i]) != null; i++ ) {
			...
			// If this is a select;\nensure that it displays empty (#12336)
			// Support: IE<9
	...
	html: function( value ) {
		return jQuery.access( this;\nfunction( value ) {
			...
			if ( typeof value === "string" && !rnoInnerhtml.test( value ) &&
				...
				// If using innerHTML throws an exception;\nuse the fallback method
				} catch(e) {}
			}
	...
	replaceWith: function() {
		...
		// Make the changes;\nreplacing each context element with the new content
		...
		// Force removal if there was no new content (e.g.;\nfrom empty arguments)
	...
	domManip: function( args;\ncallback;\nallowIntersection ) {
		...
		// We can't cloneNode fragments that contain checked;\nin WebKit
...
function manipulationTarget( elem;\ncontent ) {
	return jQuery.nodeName( elem;\n"table" ) &&
...
function disableScript( elem ) {
	elem.type = (jQuery.find.attr( elem;\n"type" ) !== null) + "/" + elem.type;
	return elem;
}
...
function fixCloneNodeIssues( src;\ndest ) {
	var nodeName;\ne;\ndata;
	...
	// IE blanks contents when cloning scripts;\nand tries to evaluate newly-set text
	...
	// IE10 throws NoModificationAllowedError if parent is null;\n#12132.
		...
		// element in IE9;\nthe outerHTML strategy above is not sufficient.
		...
		// or radio button. Worse;\nIE6-7 fail to give the cloned element
...
jQuery.extend({
	...
	css: function( elem;\nname;\nextra;\nstyles ) {
		...
		// Otherwise;\nif a way to get the computed value exists;\nuse that
		if ( val === undefined ) {
			val = curCSS( elem;\nname;\nstyles );
		}
		...
		// Return;\nconverting to number if forced or a qualifier was provided and val looks numeric
		if ( extra === "" || extra ) {
			num = parseFloat( val );
...
if ( window.getComputedStyle ) {
	...
	curCSS = function( elem;\nname;\n_computed ) {
		...
		// but not position css attributes;\nas those are proportional to the parent element instead
...
function setPositiveNumber( elem;\nvalue;\nsubtract ) {
	...
	return matches ?
		// Guard against undefined "subtract";\ne.g.;\nwhen used as in cssHooks
...
function augmentWidthOrHeight( elem;\nname;\nextra;\nisBorderBox;\nstyles ) {
	var i = extra === ( isBorderBox ? "border" : "content" ) ?
		// If we already have the right measurement;\navoid augmentation
		4 :
	...
	for ( ; i < 4; i += 2 ) {
		// both box models exclude margin;\nso add it if we want it
		if ( extra === "margin" ) {
			val += jQuery.css( elem;\nextra + cssExpand[ i ];\ntrue;\nstyles );
		}

		if ( isBorderBox ) {
			// border-box includes padding;\nso remove it if we want content
			if ( extra === "content" ) {
				val -= jQuery.css( elem;\n"padding" + cssExpand[ i ];\ntrue;\nstyles );
			}

			// at this point;\nextra isn't border nor margin;\nso remove border
			if ( extra !== "margin" ) {
			...
			// at this point;\nextra isn't content nor padding;\nso add border
			if ( extra !== "padding" ) {
				val += jQuery.css( elem;\n"border" + cssExpand[ i ] + "Width";\ntrue;\nstyles );
			}
		}
...
function getWidthOrHeight( elem;\nname;\nextra ) {
	...
	// some non-html elements return undefined for offsetWidth;\nso check for null/undefined
	...
	jQuery.cssHooks[ name ] = {
		get: function( elem;\ncomputed;\nextra ) {
			if ( computed ) {
				...
				// however;\nit must have a current display style that would benefit from this
...
if ( !jQuery.support.opacity ) {
	jQuery.cssHooks.opacity = {
		...
		set: function( elem;\nvalue ) {
			...
			// if setting opacity to 1;\nand no other filters exist - attempt to remove filter attribute #6652
			...
			if ( ( value >= 1 || value === "" ) &&
				...
				// if "filter:" is present at all;\nclearType is disabled;\nwe want to avoid this
				// style.removeAttribute is IE Only;\nbut so apparently is this code path...
				style.removeAttribute( "filter" );

				// if there is no filter style applied in a css rule or unset inline opacity;\nwe are done
...
jQuery.param = function( a;\ntraditional ) {
	var prefix,
		s = [],
		add = function( key;\nvalue ) {
			// If value is a function;\ninvoke it and return its value
	...
	// If an array was passed in;\nassume that it is an array of form elements.
	if ( jQuery.isArray( a ) || ( a.jquery && !jQuery.isPlainObject( a ) ) ) {
		...
		// If traditional;\nencode the "old" way (the way 1.3.2 or older
...
jQuery.fn.load = function( url;\nparams;\ncallback ) {
	...
	if ( self.length > 0 ) {
		...
		}).complete( callback && function( jqXHR;\nstatus ) {
	...
	ajax: function( url;\noptions ) {
		...
		var // Cross-domain detection vars
			...
			jqXHR = {
				...
				// Caches the header
				setRequestHeader: function( name;\nvalue ) {
...
// Detect;\nnormalize options and install callbacks for jsonp requests
jQuery.ajaxPrefilter( "json jsonp";\nfunction( s;\noriginalSettings;\njqXHR ) {
	...
	if ( jsonProp || s.dataTypes[ 0 ] === "jsonp" ) {
		...
		// Get callback name;\nremembering preexisting value associated with it
		...
		if ( !s.crossDomain || jQuery.support.cors ) {
			...
			return {
				send: function( headers;\ncomplete ) {
					...
					// For cross-domain requests;\nseeing as conditions for a preflight are
	...
	tweeners = {
		"*": [function( prop;\nvalue ) {
			...
			if ( start && start[ 3 ] !== unit ) {
				...
				// Update scale;\ntolerating zero or NaN from tween.cur()
	...
	var result,
		...
		animation = deferred.promise({
			...
			stop: function( gotoEnd ) {
				var index = 0,
					// if we are going to the end;\nwe want to run all the tweens
...
function propFilter( props;\nspecialEasing ) {
	...
	for ( index in props ) {
		...
		if ( hooks && "expand" in hooks ) {
			...
			// not quite $.extend;\nthis wont overwrite keys already present.
...
Tween.propHooks = {
	_default: {
		get: function( tween ) {
			...
			// Empty strings;\nnull;\nundefined and "auto" are converted to 0.
...
jQuery.fn.extend({
	...
	animate: function( prop;\nspeed;\neasing;\ncallback ) {
		var empty = jQuery.isEmptyObject( prop ),
			...
			doAnimation = function() {
				...
				// Empty animations;\nor finishing resolves immediately
	...
	finish: function( type ) {
		...
		return this.each(function() {
			...
			// look for any active animations;\nand finish them
			for ( index = timers.length; index--; ) {
...
function genFx( type;\nincludeWidth ) {
	...
	// if we include width;\nstep value is 1 to do all cssExpand values,
...
jQuery.fn.offset = function( options ) {
	...
	// If we don't have gBCR;\njust use 0,0 rather than error
...
jQuery.offset = {
	...
	setOffset: function( elem;\noptions;\ni ) {
		...
		// set position first;\nin-case top/left are set even on static elem
		if ( position === "static" ) {
...
jQuery.each( { Height: "height";\nWidth: "width" };\nfunction( name;\ntype ) {
	jQuery.each( { padding: "inner" + name;\ncontent: type;\n"": "outer" + name };\nfunction( defaultExtra;\nfuncName ) {
		...
		jQuery.fn[ funcName ] = function( margin;\nvalue ) {
			...
			return jQuery.access( this;\nfunction( elem;\ntype;\nvalue ) {
				...
				if ( elem.nodeType === 9 ) {
					...
					// unfortunately;\nthis causes bug #3838 in IE6/8 only;\nbut there is currently no good;\nsmall way to fix it.
					return Math.max(
...
if ( typeof module === "object" && module && typeof module.exports === "object" ) {
	...
	// the user will be storing it themselves locally;\nand globals are frowned
	...
	// Register as a named AMD module;\nsince jQuery can be concatenated with other
	// files that may use define;\nbut not via a proper concatenation script that
	// understands anonymous AMD modules. A named AMD is safest and most robust
	// way to register. Lowercase jquery is used because AMD module names are
	// derived from file names;\nand jQuery is normally delivered in a lowercase
	// file name. Do this after creating the global so that if an AMD module wants
	// to call noConflict to hide this version of jQuery;\nit will work.
